...
def _build_report_data_dict(session: ScanSessionResult) -> dict:
    ...
    return {
        ...
        "suppressed_alerts": [
            _serialize_suppressed_alert(alert) for alert in session.suppressed_alerts
        ],
        "scanner_host_ip": session.scanner_host_ip,
    }
...
def _serialize_suppressed_alert(alert: ThreatAlertRecord) -> dict:
    return {
        "alert_id": alert.alert_id,
        "source_ip": alert.source_ip,
        "destination_ip": alert.destination_ip,
        "threat_category": alert.threat_category.value,
        "original_risk_level": alert.original_risk_level.value if alert.original_risk_level else "",
        "confidence_score": round(alert.confidence_score, 3),
        "description": alert.description,
        "false_positive_reason": alert.false_positive_reason,
        "suppressed_from_risk_score": alert.suppressed_from_risk_score,
        "mitre_technique_id": alert.mitre_technique_id,
        "timestamp": alert.timestamp,
    }
...
def _render_html_report(report_data: dict, session: ScanSessionResult) -> str:
    ...
    suppressed_alert_list = report_data.get("suppressed_alerts", [])
    scanner_host_ip = report_data.get("scanner_host_ip", "")
    ...
    suppressed_rows = ""
    for suppressed_alert in suppressed_alert_list:
        original_level = suppressed_alert.get("original_risk_level", "")
        original_color = risk_color_map.get(original_level, "#6b7280")
        suppressed_rows += f"""
        <tr>
            <td><code>{suppressed_alert['source_ip']}</code></td>
            <td>{suppressed_alert['threat_category']}</td>
            <td style="color:{original_color}">{original_level}</td>
            <td style="font-size:0.85em;color:#94a3b8">{suppressed_alert['description'][:120]}</td>
            <td style="font-size:0.8em;color:#fbbf24">{suppressed_alert['false_positive_reason']}</td>
        </tr>"""
...
<div class="layout">
  <nav class="sidebar">
    ...
    <a href="#suppressed">&#x2714; Authorized Activity
      <span class="nav-count">{len(suppressed_alert_list)}</span>
    </a>
  ...
  <div class="container">
    ...
    <div class="section" id="summary">
      ...
      <div class="kpi-grid">
        ...
        <div class="kpi-card">
          <div class="kpi-value" style="color:#64748b">{len(suppressed_alert_list)}</div>
          <div class="kpi-label">Suppressed (Authorized)</div>
        </div>
      </div>
      <p style="color:#94a3b8;margin-top:8px">{summary['summary_text']}</p>
      {f'<p style="color:#64748b;font-size:0.85em;margin-top:6px">&#x2714; Scanner host <code>{scanner_host_ip}</code> — self-generated traffic suppressed from risk scoring.</p>' if scanner_host_ip else ''}
    </div>
    ...
    <div class="section" id="suppressed">
      <div class="section-title">Authorized / Suppressed Activity ({len(suppressed_alert_list)})</div>
      {'<p style="color:#64748b">No scanner-generated alerts suppressed.</p>' if not suppressed_alert_list else f'''
      <p style="color:#94a3b8;font-size:0.85em;margin-bottom:12px">
        These alerts were detected but excluded from risk scoring because they were generated by
        the authorized scanner host{f" (<code>{scanner_host_ip}</code>)" if scanner_host_ip else ""}.
        They are preserved here for audit and review.
      </p>
      <table>
        <thead><tr>
          <th>Source IP</th><th>Category</th><th>Original Severity</th>
          <th>Description</th><th>Suppression Reason</th>
        </tr></thead>
