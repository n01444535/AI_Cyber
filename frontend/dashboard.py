...
def _render_platform_header() -> None:
    ...
    header_text.append(
        "Autonomous Threat Detection & Behavioral Security Analysis", style="dim white"
    )
...
def _render_executive_kpis(session: ScanSessionResult) -> None:
    critical_count = sum(
        1 for p in session.host_risk_profiles if p.risk_level == RiskLevel.CRITICAL
    )
    ...
    kpi_panels = [
        Panel(
            f"[bold white]{len(session.scanned_hosts) or len(session.host_risk_profiles)}[/bold white]",
            title="Hosts",
            border_style="blue",
        ),
        Panel(f"[critical]{critical_count}[/critical]", title="Critical", border_style="red"),
        Panel(f"[high]{high_count}[/high]", title="High Risk", border_style="orange3"),
        Panel(
            f"[bold white]{len(session.threat_alerts)}[/bold white]",
            title="Alerts",
            border_style="yellow",
        ),
        Panel(
            f"[bold white]{len(session.attack_stories)}[/bold white]",
            title="Attack Stories",
            border_style="magenta",
        ),
        Panel(
            f"[bold white]{anomaly_count}[/bold white]", title="ML Anomalies", border_style="cyan"
        ),
        Panel(
            f"[bold white]{session.total_risk_score:.0f}[/bold white]",
            title="Risk Score",
            border_style="red" if session.total_risk_score >= 80 else "yellow",
        ),
    ]
...
def _render_top_risk_hosts(host_profiles: list[HostRiskProfile]) -> None:
    ...
    for profile in host_profiles:
        ...
        explanation_short = (
            profile.ai_explanation[:80] + "…"
            if len(profile.ai_explanation) > 80
            else profile.ai_explanation
        )
...
def _render_threat_alerts(threat_alerts: list[ThreatAlertRecord]) -> None:
    if not threat_alerts:
        _console.print(
            Panel(
                "[low]No threats detected.[/low]",
                title="[header]Threat Alerts[/header]",
                border_style="green",
            )
        )
        return
...
def _render_mitre_coverage(threat_alerts: list[ThreatAlertRecord]) -> None:
    ...
    for tactic_name, tactic_count in sorted(
        tactic_counts.items(), key=lambda x: x[1], reverse=True
    ):
        related_techniques = [
            tech_key
            for tech_key in technique_counts
            if any(
                alert.mitre_tactic_name == tactic_name
                and f"{alert.mitre_technique_id} " in tech_key
                for alert in threat_alerts
            )
        ]
        mitre_table.add_row(
...
def _render_host_detail_panel(profile: HostRiskProfile) -> None:
    ...
    detail_text.append(
        f"  |  Critical Ports: {profile.critical_port_count}",
        style="critical" if profile.critical_port_count > 0 else "white",
    )
    detail_text.append("  |  ML Anomaly: ", style="white")
    detail_text.append(
        "YES" if profile.is_ml_anomaly else "No",
    ...
    if profile.threat_alerts:
        ...
        for alert in profile.threat_alerts[:5]:
            ...
            detail_text.append(
                f"{alert.threat_category.value}: {alert.description[:80]}\n", style="white"
            )
